...
signed main() {
    ...
    while (t--) {
        ...
        vi fenwick(n+1,0), start(n+1), end(n+1), lt(n+1);
        int currTime=0;
        dfs(adj, 1, 0, start, end, a, lt, currTime);
        for(int i=1;i<=n;i++){
            addFT(fenwick, start[i], a[i-1]);
        }
        while(q--){
            int typ;
            cin>>typ;
            if(typ==1){
                int node, val;
                cin>>node>>val;
                addFT(fenwick, start[node], val-a[node-1]);
                a[node-1]=val;
            }
            else{
                int node;
                cin>>node;
                int res=query(fenwick, end[node])-query(fenwick, start[node]-1);
                cout<<res<<endl;
            }
        }
